...
def get_model_flops(model, input_res, print_per_layer_stat=True,
    ...
    if input_constructor:
        ...
        # _ = flops_model(batch)
        image_input = torch.FloatTensor(1, *input_res[0]).to(device)  # image input
        event_input = torch.FloatTensor(1, *input_res[1]).to(device)  # event input
        _ = flops_model(image_input, event_input)  # two inputs
