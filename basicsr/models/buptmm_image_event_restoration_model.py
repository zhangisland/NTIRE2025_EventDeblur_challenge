...
logger = get_root_logger('basicsr')
class BUPTMMImageEventRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if load_path is not None:
            ...
            current_ev_chn = self.opt['network_g'].get('ev_chn', 24)  # Default to current model's channels
            pretrained_ev_chn = self.opt['path'].get('pretrained_ev_chn', 6)  # Default to pretrained model's channels
            
            self.load_network_with_ev_converter(self.net_g, load_path, current_ev_chn=current_ev_chn,
                                                   pretrained_ev_chn=pretrained_ev_chn,
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('perceptual_opt'):
            self.percep_type = train_opt['perceptual_opt'].pop('type')
            cri_perceptual_cls = getattr(loss_module, self.percep_type)
        ...
        if train_opt.get('charbonnier_opt'):
            self.charbonnier_type = train_opt['charbonnier_opt'].pop('type')
            cri_charbonniner_cls = getattr(loss_module, self.charbonnier_type)
            self.cri_charbonnier = cri_charbonniner_cls(**train_opt['charbonnier_opt']).to(self.device)
        else:
            self.cri_charbonnier = None
        
        if train_opt.get('sobel_opt'):
            self.sobel_type = train_opt['sobel_opt'].pop('type')
            cri_sobel_cls = getattr(loss_module, self.sobel_type)
            self.cri_sobel = cri_sobel_cls(**train_opt['sobel_opt']).to(self.device)
        else:
            self.cri_sobel = None


        if (self.cri_pix is None) and (self.cri_perceptual is None) and (self.cri_charbonnier is None) and (self.cri_sobel is None):
            raise ValueError('Pixel(PSNR), perceptual, Charbonnier, sobel losses are None.')
    ...
    def feed_data(self, data):
        ...
        logger = get_root_logger()
        logger.debug(f'lq.shape={self.lq.shape}, self.voxel.shape={self.voxel.shape}')
        if 'mask' in data:
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_charbonnier:
            l_charbonnier = self.cri_charbonnier(self.output, self.gt)
            l_total += l_charbonnier
            loss_dict['l_charbonnier'] = l_charbonnier
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        dataset_name = self.opt.get('dataset_name')
            ...
            if save_img:                
                if self.opt['is_train']:
                    if cnt == 1: # visualize cnt=1 image every time
                        ...
                        # logger.info(f"self.opt['path']['visualization']={self.opt['path']['visualization']}")
                        
                        save_img_path = osp.join(self.opt['path']['visualization'],
                                                f'train_vis_{dataset_name}',
                                                f'{self.image_name}_{current_iter:08d}.png')
                        
                        save_gt_img_path = osp.join(self.opt['path']['visualization'],
                                                f'train_vis_{dataset_name}',
                                                f'{self.image_name}_{current_iter:08d}_gt.png')
                        logger.info(f'save_img_path={save_img_path}')
                else:
                ...
                if not (self.opt['is_train'] and cnt == 1):
                    pass
                else:
                    if not osp.exists(osp.dirname(save_img_path)):
                        os.makedirs(osp.dirname(save_img_path))
                        
                    if not osp.exists(osp.dirname(save_gt_img_path)):
                        os.makedirs(osp.dirname(save_gt_img_path))
        ...
        if with_metrics:
            for metric in self.metric_results.keys():
                ...
                if metric == 'psnr':
                    if self.metric_results['psnr'] > self.best_psnr:
                        self.best_psnr = self.metric_results['psnr']
                        logger.info(f'Saving model with best psnr={self.best_psnr} at [iter={current_iter:08d}] ')
                    self.save_best_model(net=self.net_g, 
                                            net_label='net_g',
                                            current_iter=current_iter,
                                            best_metric_value=self.metric_results[metric], 
                                            metric_name='psnr')                
                elif metric == 'ssim':
                    if self.metric_results['ssim'] > self.best_ssim:
                        self.best_ssim = self.metric_results['ssim']
                        logger.info(f'Saving model with best ssim={self.best_ssim} at [iter={current_iter:08d}] ')
                    self.save_best_model(net=self.net_g, 
                                         net_label='net_g',
                                         current_iter=current_iter,
                                         best_metric_value=self.metric_results[metric], 
