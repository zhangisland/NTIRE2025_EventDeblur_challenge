...
class BUPTMMTestImageEventRestorationModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        if isinstance(self.seq_name, list):
            self.seq_name = self.seq_name[0]
        if isinstance(self.img_name, list):
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            pbar.update(1)
            pbar.set_description(f'Test {self.img_name}')
            cnt += 1
            # if cnt == 300:
            #     break
        pbar.close()
