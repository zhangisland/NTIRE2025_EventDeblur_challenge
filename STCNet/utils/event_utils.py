...
def image_proess(inp_img, inp_event, tar_img, ps):
    # 打印输入图像、事件数据和目标图像的初始维度
    #print(f"Before padding: inp_img shape = {inp_img.shape}, tar_img shape = {tar_img.shape}, inp_event shape = {inp_event.shape}")
    ...
    # 打印填充后的尺寸
    #print(f"After padding: inp_img shape = {inp_img.shape}, tar_img shape = {tar_img.shape}, inp_event shape = {inp_event.shape}")
    ...
    #print(f"hh: {hh}, ww: {ww}, ps: {ps}")
    rr = random.randint(0, hh - ps)
    cc = random.randint(0, ww - ps)
    #print(f"Generated rr: {rr}, cc: {cc}")
    ...
    # 打印裁剪后的尺寸
    #print(f"After crop: inp_img shape = {inp_img.shape}, tar_img shape = {tar_img.shape}, inp_event shape = {inp_event.shape}")
